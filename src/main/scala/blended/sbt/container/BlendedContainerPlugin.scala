...
object BlendedContainerPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val materializeContainerFiles = taskKey[Seq[File]]("List of additional directories, which content will be placed into the container directory")
  ...
  override def projectSettings: Seq[Def.Setting[_]] = super.projectSettings ++ Seq(
    ...
    materializeContainerFiles := Seq((Compile / filterTargetDir).value / "container"),
    ...
    packageFullNoJreMapping := {
      ...
      PathFinder(launcherDir).allPaths.pair(MappingsHelper.relativeTo(launcherDir)).
        ...
        containerResources.flatMap(cr => PathFinder(cr).allPaths.pair(MappingsHelper.relativeTo(containerResources))) ++
    ...
    packageFullJreLinuxMapping := {
      ...
      val jreTarget = target.value / "unpackedJreLinux"
      if (jreTarget.exists()) IO.delete(jreTarget)
      jreTarget.mkdirs()

      val realJreDir = prefix match {
        case None => jreTarget
        case Some(prefix) => jreTarget / prefix
      }

      // unpack
      sys.process.Process(
        command = Seq("tar", "xzf", jre.getAbsolutePath()),
        cwd = jreTarget
      ).!

      // mapping
      packageFullNoJreMapping.value ++
        PathFinder(realJreDir).allPaths.pair(f =>
          IO.relativize(realJreDir, f).map(p => s"jre/${p}"))
    },

    packageFullJreLinuxArtifact := Artifact(
      name = moduleName.value,
      `type` = "tar.gz",
      extension = "tar.gz",
      classifier = "full-jre-linux"
    ),

    packageFullJreWindowsArtifact := Artifact(
      name = moduleName.value,
      `type` = "zip",
      extension = "zip",
      classifier = "full-jre-windows"
    ),
    ...
    packageFullJreWindowsMapping := {
      ...
      val jreTarget = target.value / "unpackedJreWindows"
      if (jreTarget.exists()) IO.delete(jreTarget)
      jreTarget.mkdirs()
